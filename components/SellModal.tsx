...
const SellModal: React.FC<SellModalProps> = ({ onClose }) => {
  ...
  const leafCategories = useMemo(() => getLeafCategories(CATEGORIES), []);
  ...
  const handleSuggestCategory = useCallback(async () => {
    ...
    try {
      ...
      setError(t('error_suggestion_failed'));
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <form className="p-6 space-y-4">
          ...
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
           ...
           <div>
            ...
            <select id="category" value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-slate-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select id="condition" value={condition} onChange={(e) => setCondition(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-slate-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                  <option>New</option>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <div className="relative mt-1">
                ...
                <input type="number" id="price" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="0.00" className="block w-full rounded-md border-slate-300 pl-7 pr-3 focus:border-blue-500 focus:ring-blue-500 sm:text-sm" />
              </div>
            </div>
            <div>
        ...
        <div className="flex justify-end items-center p-4 bg-slate-50 border-t space-x-2 sticky bottom-0">
          <button onClick={onClose} className="px-4 py-2 border border-slate-300 text-sm font-medium rounded-md text-slate-700 bg-white hover:bg-slate-50 focus:outline-none">{t('cancel')}</button>
          <button className="px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none">{t('list_for_sale')}</button>
        </div>
      </div>
    </div>
  );
